function [grid_data] = amg_grids_setup3Dx(A, i_method, max_levels)
...
%
% amg_grids_setup3Dx(A, i_method, max_levels)
%
...
if iout>1 % display pattern of connections
        figure(13)
        subplot(121), spy(A,'ro'), hold on, spy(is_negative,'ko'),
        title('negative connections')
        subplot(122), spy(is_negstrong,'bo'),
        title('strong negative connections'), pause
end
...
%------------------------------ optimised code starts
        nneg = nnz(is_negstrong);
        [r,c]=find(is_negstrong);
        for index=1:nneg % run through the negative connections
...
%
        nweak = nnz(is_negative);
        [r,c]=find(is_negative);
        %------ replacement code
        vv=full(A(A<0)); Aneg=sparse(r,c,vv,s_A,s_A,nnz_A);
        sum_weak=full((sum(Aneg'))'); n_weak=full((sum(is_negative'))');
%
       npos = nnz(is_positive);
       [r,c]=find(is_positive);
       for index=1:npos % run through the positive connections
          i = r(index); j = c(index);
          if j ~= i % ignore diagonal entries
             if p_flag == false
             % Set up stuff needed for +ve connections
             p_flag = true;
...
%        pt_type = zeros(s_A,1); %<------ sparse
        ...
        % First pass 
        % ---------- starting values of grid weights
        g_weight = uint32(full(sparse(n_str_t)));  % <--- uint32
        w_inc = 1; % amount to increase g_weight for each strong connection to an F point
        % w_inc=1 in BHM & 2 in Stuben
           
        
        % find point with maximum value of g_weight
        [max_wt, max_pos] = max(g_weight); % whos
...
%         end
        ...
        if i_method < 2 % & level > 2 %%----- speed up
                    ...
                    if n_fails > 0
                        ...
                        for index_k = 1:n_str_t(new_C)
                            ...
                            if pt_type(pt_k) == 1 
                                ...
                                str_F(:,pt_k) = 0;
                                str_F( 1:n_str_F(pt_k),pt_k ) = int_setdiff( str( 1:n_str(pt_k),pt_k ), str_C( 1:n_str_C(pt_k),pt_k ) );
                            end
                        end
                    end
                end
                ...
                if pt_type(C_p) ~= 2
                    ...
                    for index = 1:n_str_t(C_p) % run through negative strong connections
                        ...
                        if pt_type(pt_i) == 1 % for fine points
                            ...
                            str_F(:,pt_i) = 0;
                            str_F( 1:n_str_F(pt_i),pt_i ) = int_setdiff( str( 1:n_str(pt_i),pt_i ), str_C( 1:n_str_C(pt_i),pt_i ) );
                        end
                    end
                end
